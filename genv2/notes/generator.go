...
func (m *Model) setClassModel(metaPath string, isChildIteration bool, definitions Definitions, parents, pkgNames, mainParentChildren, parentHierarchyList []string) {
	...
	m.ResourceClassName = strings.ToUpper(m.PkgName[:1]) + m.PkgName[1:] // used for naming the resource functions in the provider
	m.Definitions = definitions                                          // unneeded each class has it's own defintion included
	m.Configuration = GetClassConfiguration(m.PkgName, definitions)      // TODO determine if needed and how to expose global configuration

	for _, classDetails := range classInfo {
		m.SetClassLabel(classDetails)                         // unneeded created for resource name creation could be cleaned
		m.SetClassName(classDetails)                          // unneeded already set during read of meta file, also not really used
		m.SetRelationshipClasses(definitions)                 // TODO determine if needed was used for documentation but should be deduced from metadata
		m.SetClassRnFormat(classDetails)                      // used add added rnformat
		m.SetClassRnFormatList(classDetails)                  // TODO determine if needed was used for documentation but should be deduced from metadata
		m.SetClassDnFormats(classDetails)                     // used and added dnformats, not the documentation dnformats seems redundant
		m.SetClassIdentifiers(classDetails)                   // used and added identifiedBy, no overwrites yet
		m.SetClassInclude()                                   // TODO determine if overwrite needed
		m.SetClassExclude()                                   // TODO determine if overwrite needed
		m.SetClassAllowDelete(classDetails)                   // Added, AllowClassDelete overwrite seems not needed and should be cleaned since it is not used
		m.SetClassContainedByAndParent(classDetails, parents) // Added, hasParents could be cleaned
		m.SetClassContains(classDetails)                      // Added, used for deteremining children in docs to be added
		m.SetClassComment(classDetails)                       // TODO determine if needed seems not used
		m.SetClassVersions(classDetails)                      // Added just the full string
