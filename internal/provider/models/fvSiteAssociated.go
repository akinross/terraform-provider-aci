...
func (m *FvSiteAssociatedModel) BuildPayloadObject(
	...
	if !m.FvRemoteId.IsNull() && !m.FvRemoteId.IsUnknown() {
		var desiredChildren []FvRemoteIdModel
		diagnostics.Append(m.FvRemoteId.ElementsAs(ctx, &desiredChildren, false)...)
		if diagnostics.HasError() {
			return nil, diagnostics
		}

		var priorChildren []FvRemoteIdModel
		if priorState != nil &&
			!priorState.FvRemoteId.IsNull() &&
			!priorState.FvRemoteId.IsUnknown() {
			diagnostics.Append(priorState.FvRemoteId.ElementsAs(ctx, &priorChildren, false)...)
			if diagnostics.HasError() {
				return nil, diagnostics
			}
		}
		...
		for desiredIndex := range desiredChildren {
			...
			var priorChild *FvRemoteIdModel
			...
			children = append(children, map[string]any{"fvRemoteId": childObject})
		}
		...
		for priorIndex := range priorChildren {
			...
			children = append(children, map[string]any{"fvRemoteId": priorChild.BuildNestedDeletePayloadObject()})
		}
	}
...
func (m *FvSiteAssociatedModel) BuildNestedDeletePayloadObject() map[string]any {
...
func (m *FvSiteAssociatedResourceModel) BuildDeletePayload() (*container.Container, diag.Diagnostics) {
	var diagnostics diag.Diagnostics
	payload, err := json.Marshal(map[string]any{
		"fvSiteAssociated": map[string]any{
			"attributes": map[string]any{
				"dn":     m.ID.ValueString(),
				"status": "deleted",
			},
