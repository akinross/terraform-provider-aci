...
func (m *FvRsBdToEpRetModel) BuildPayloadObject(
	...
	if !m.ResolveAct.IsNull() && !m.ResolveAct.IsUnknown() {
		attributes["resolveAct"] = m.ResolveAct.ValueString()
	}
	if !m.TnFvEpRetPolName.IsNull() && !m.TnFvEpRetPolName.IsUnknown() {
		attributes["tnFvEpRetPolName"] = m.TnFvEpRetPolName.ValueString()
	}
