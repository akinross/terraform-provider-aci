...
func (m *CoppIfPolModel) BuildPayloadObject(
	...
	if !m.CoppProtoClassP.IsNull() && !m.CoppProtoClassP.IsUnknown() {
		var desiredChildren []CoppProtoClassPModel
		diagnostics.Append(m.CoppProtoClassP.ElementsAs(ctx, &desiredChildren, false)...)
		if diagnostics.HasError() {
			return nil, diagnostics
		}

		var priorChildren []CoppProtoClassPModel
		if priorState != nil &&
			!priorState.CoppProtoClassP.IsNull() &&
			!priorState.CoppProtoClassP.IsUnknown() {
			diagnostics.Append(priorState.CoppProtoClassP.ElementsAs(ctx, &priorChildren, false)...)
			if diagnostics.HasError() {
				return nil, diagnostics
			}
		}
		...
		for desiredIndex := range desiredChildren {
			...
			var priorChild *CoppProtoClassPModel
			...
			children = append(children, map[string]any{"coppProtoClassP": childObject})
		}
		...
		for priorIndex := range priorChildren {
			...
			children = append(children, map[string]any{"coppProtoClassP": priorChild.BuildNestedDeletePayloadObject()})
		}
	}
...
func (m *CoppIfPolModel) BuildNestedDeletePayloadObject() map[string]any {
...
func (m *CoppIfPolResourceModel) BuildDeletePayload() (*container.Container, diag.Diagnostics) {
	var diagnostics diag.Diagnostics
	payload, err := json.Marshal(map[string]any{
		"coppIfPol": map[string]any{
			"attributes": map[string]any{
				"dn":     m.ID.ValueString(),
				"status": "deleted",
			},
