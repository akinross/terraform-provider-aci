...
func (m *MgmtInstPModel) BuildPayloadObject(
	...
	if !m.MgmtRsOoBCons.IsNull() && !m.MgmtRsOoBCons.IsUnknown() {
		var desiredChildren []MgmtRsOoBConsModel
		diagnostics.Append(m.MgmtRsOoBCons.ElementsAs(ctx, &desiredChildren, false)...)
		if diagnostics.HasError() {
			return nil, diagnostics
		}

		var priorChildren []MgmtRsOoBConsModel
		if priorState != nil &&
			!priorState.MgmtRsOoBCons.IsNull() &&
			!priorState.MgmtRsOoBCons.IsUnknown() {
			diagnostics.Append(priorState.MgmtRsOoBCons.ElementsAs(ctx, &priorChildren, false)...)
			if diagnostics.HasError() {
				return nil, diagnostics
			}
		}
		...
		for desiredIndex := range desiredChildren {
			...
			var priorChild *MgmtRsOoBConsModel
			...
			children = append(children, map[string]any{"mgmtRsOoBCons": childObject})
		}
		...
		for priorIndex := range priorChildren {
			...
			children = append(children, map[string]any{"mgmtRsOoBCons": priorChild.BuildNestedDeletePayloadObject()})
		}
	}
...
func (m *MgmtInstPModel) BuildNestedDeletePayloadObject() map[string]any {
...
func (m *MgmtInstPResourceModel) BuildDeletePayload() (*container.Container, diag.Diagnostics) {
	var diagnostics diag.Diagnostics
	payload, err := json.Marshal(map[string]any{
		"mgmtInstP": map[string]any{
			"attributes": map[string]any{
				"dn":     m.ID.ValueString(),
				"status": "deleted",
			},
